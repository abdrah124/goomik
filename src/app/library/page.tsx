...
import { Box, IconButton, InputBase, Paper, Stack, Typography, alpha, styled } from "@mui/material";
...
const items = sortArray(libraryItems).filter(item => item.includes(input))
  ...
  return (
    <div className="w-full">
      <Paper elevation={5} sx={{width:"100%", display:"flex",backgroundColor:"inherit", justifyContent:"space-between", px:2, mb:3, py:1,}}>
      <Search
      ...
      {
list ? <Stack direction="column" gap={2} alignItems="center">
        {libraryItems.length > 0 &&
          items.map((item) => <LibraryCard key={item} id={item} />)}
      </Stack> : <CardGrid>
        {libraryItems.length > 0 && items.map(item => <MangaLibraryCard key={item}id={item} />)}
      </CardGrid>
      }
